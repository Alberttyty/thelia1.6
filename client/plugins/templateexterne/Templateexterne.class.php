<?php
require_once(realpath(dirname(__FILE__)) . "/../../../classes/parseur/Analyse.class.php");
require_once(realpath(dirname(__FILE__)) . "/lib/simple_html_dom.php");
...
class Templateexterne extends PluginsClassiques {
	...
	function Templateexterne() {
		...
		$this->cache_dir = SITE_DIR."client/cache/externe/";
		$this->cache_vie = 3600;
	}     
    	...
    	global $res;
      	...
      	if (preg_match_all('/#TEMPLATE_EXTERNE_([^{]+){([^}]+),([^}]+)}/', $res, $matches, PREG_SET_ORDER)) {
        	...
        	if(isset($_REQUEST['var_mode'])) {
				if($_REQUEST['var_mode']=="recalcul") $forcer_recalculer=true;
			}
			...
			foreach($matches as $match)	{
				...
				$externe=$this->chargerExterneAvecCache($url,$chemin,$balise,$forcer_recalculer);
    ...
    function chargerExterneAvecCache($url,$chemin='',$balise,$forcer_recalculer=false){
