<?php
	...
	class Exapaq extends PluginsTransports{


		function Exapaq(){
			$this->PluginsTransports("exapaq");
		}
		
		function init(){
			$this->ajout_desc("Exapaq", "Exapaq", "", 1);
			$test = new Message();
			if(! $test->charger("exapaq")){
				$message = new Message();
				$message->nom = "exapaq";
				$lastid = $message->add();
    ...
    function calcule(){
      require_once(SITE_DIR."/client/plugins/exapaq/config.php");
      return exapaq_calcul($this->zone,$this->nbart,$this->total,$this->poids);
		}
