...
if(!@class_exists('SystempayApi', false)) {
	...
	class SystempayApi {
		 ...
		 * @var SystempayResponse
		 * @access private
		 * @deprecated see SystempayResponse constructor
		 */
		var $response;
		/**
		...
		*/
		 ...
		 */
		function SystempayApi($encoding="UTF-8") {
		...
		function _addRequestField($name, $label, $regex, $required = false,
			...
			$this->requestParameters[$name] = new SystempayField($name, $label, $regex,
					$required, $length);
		...
		function getSupportedCurrencies() {
			...
			foreach($currencies as $currency) {
				$systempayCurrencies[] = new SystempayCurrency($currency[0], $currency[1], $currency[2]);
			}
		...
		*/
		 ...
		 * @param array[string]SystempayField $fields
		 * @return string
		 * @access private
		 */
		 ...
		 * @return array[string]SystempayField a list of SystempayField or false if a parameter was invalid
		 * @see SystempayField
		 */
		...
		function loadResponse($parameters = null, $ctx_mode = null, $key_test = null,
			...
			$this->response = new SystempayResponse();
			$this->response->load($parameters, $this->getCertificate());
		}
	
		/**
		 * Return a SystempayResponse object representing the result of the payment.
		 * You can provide arguments to load data as in loadResponse.
		 * @see SystempayApi::loadResponse
		 * @return SystempayResponse
		 * @deprecated see SystempayResponse constructor
		 */
...
if(!@class_exists('SystempayResponse', false)) {
	...
	class SystempayResponse {
		...
		/**
		 * Internal reference to SystempayApi for using util methods
		 * @var SystempayApi
		 * @access private
		 */
		var $api;
		 ...
		 */
		function SystempayResponse($parameters = null, $ctx_mode = null, $key_test = null, $key_prod = null, $encoding = 'UTF-8') {
				
			$this->api = new SystempayApi($encoding);
		...
		*/
		...
		function getOutputForGateway($case = '', $extraMessage = '') {
			...
			if($this->api->encoding !== "UTF-8") {
				$message = iconv($this->api->encoding, "UTF-8", $message);
			}
...
if(!@class_exists('SystempayField', false)) {
	...
	class SystempayField {
		 ...
		 * @param boolean $required
		 * @param string $value
		 * @return SystempayField
		 */
		function SystempayField($name, $label, $regex, $required = false, $length = 255) {
			$this->name = $name;
			$this->label = $label;
			$this->regex = $regex;
